...
bool Custom1337::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz - 6.5, square_sz - 6.5));
    ...
    //CHECKMARK BACKGHROUND
    //if (*v)
    //{
    //    window->DrawList->AddRectFilled(check_bb.Min + pad - ImVec2(4, 4), check_bb.Max - pad + ImVec2(4, 4), ImColor(255, 255, 255, 180), style.FrameRounding);
    //}
    ...
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad) - ImVec2(4, 3), check_col, square_sz - pad * 2.0f);

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 5, check_bb.Min.y + style.FramePadding.y - 2.8), (label));
